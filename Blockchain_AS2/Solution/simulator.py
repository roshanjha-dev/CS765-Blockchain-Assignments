...
class Peer:
    def __init__(self, peer_id, blockchain, speed, cpu_type, Ttx, malicious_blockchain):
        ...
        self.malicious_blockchain = malicious_blockchain
    ...
    def receive_block(self, block, sender_peer):
        ...
        success = self.malicious_blockchain.add_block(block)
        return success
...
def create_peers(n, z0, z1, Ttx, blockchain, malicious_blockchain):
    peers = []
    ...
    for i in range(n):
        peer = Peer(
            ...
            Ttx=Ttx,
            malicious_blockchain=malicious_blockchain
        )
        peers.append(peer)
...
# Set up the ring master for malicious peers
    ...
    if current_length == peer.blockchain.get_chain_length():
        ...
        if len(valid_transactions) > 2:
            if not peer.malicious:
                ...
                if len(peer.malicious_blockchain.main_chain) == 0:
                    peer.malicious_blockchain.main_chain.append(peer.blockchain.get_latest_block().block_id)
                    peer.malicious_blockchain.blocks[peer.blockchain.get_latest_block().block_id] = peer.blockchain.get_latest_block()
                    latest_block = peer.blockchain.get_latest_block()

                else:
                    latest_block = peer.malicious_blockchain.get_latest_block()
                ...
                # Add the hash mapping before adding the block
                peer.malicious_blockchain.hash_to_block_id[new_block.hash] = new_block.block_id
                ...
                success = peer.malicious_blockchain.add_block(new_block)
...
def check_selfish_mining_conditions(peer, peers, new_block, event_queue, current_time):
    ...
    malicious_chain = peer.malicious_blockchain.main_chain
...
def event_broadcast_private_chain(peer, data, event_queue, current_time):
    ...
    malicious_chain_ids = peer.malicious_blockchain.main_chain
    
    if len(malicious_chain_ids) > 0:

        # Broadcast blocks in order to ensure proper chain construction
        for block_to_broadcast in peer.malicious_blockchain.main_chain:
            block_exists = False
            ...
            if not block_exists:
                ...
                block = peer.malicious_blockchain.blocks[block_to_broadcast]
...
def event_update_honest_view(peer, data, event_queue, current_time):
...
def event_add_to_private_chain(peer, data):
    """Add a block to the private chain of a malicious peer."""
    block = data
    
    if block.block_id not in peer.block_history:
        # Add block to private chain
        peer.block_history.add(block.block_id)
        success = peer.global_malicious_blockchain.add_block(block)
        peer.received_hashes[block.hash].append(peer.peer_id)
        peer.block_history_new[block.hash] = block
        if success:
            print(f"Malicious peer {peer.peer_id} added block {block.index} to private chain")
...
def event_handle_get(peer, data, event_queue, current_time):
    ...
    #Ellipse attack
    if not peer.malicious and sender_peer.malicious:
        return
...
def event_simulate_network(n, z0, z1, Ttx, x, y, filename, Tt):
    ...
    # Create blockchains
    blockchain = Blockchain()  # Regular blockchain for honest nodes
    
    # Ensure the genesis block is properly set
    genesis_block = Block(0, "0", [], "Genesis")
    
    # Set up the genesis block in the honest blockchain
    blockchain.blocks[genesis_block.block_id] = genesis_block
    blockchain.main_chain = [genesis_block.block_id]
    blockchain.hash_to_block_id[genesis_block.hash] = genesis_block.block_id
    blockchain.children[genesis_block.block_id] = []
    ...
    malicious_blockchain = copy.deepcopy(blockchain)
    
    # Verify both blockchains have the same genesis block
    honest_genesis_id = blockchain.main_chain[0]
    malicious_genesis_id = malicious_blockchain.main_chain[0]
    
    honest_genesis = blockchain.blocks[honest_genesis_id]
    malicious_genesis = malicious_blockchain.blocks[malicious_genesis_id]
    ...
    malicious_blockchain.add_block = lambda block: fix_blockchain_add_block(malicious_blockchain, block)
    
    peers = create_peers(n, z0, z1, Ttx, blockchain, malicious_blockchain)
    ...
    print_blockchain(malicious_blockchain)
    
    # Print success metrics
    honest_blocks = len(blockchain.main_chain) - 1  # Subtract genesis block
    malicious_blocks = len(malicious_blockchain.main_chain) - 1  # Subtract genesis block

    # Save final visualizations with the new functions
    visualize_combined_blockchain(blockchain, malicious_blockchain, peers, filename)
    ...
    if honest_blocks + malicious_blocks > 0:
        ...
        malicious_blocks_in_main_chain = sum(1 for block_id in blockchain.main_chain 
                                          if block_id in blockchain.blocks and
                                          blockchain.blocks[block_id].miner_id in 
                                          [p.peer_id for p in malicious_peers])
        
        malicious_success_rate = (malicious_blocks_in_main_chain / len(blockchain.main_chain)) * 100
    ...
    return blockchain, malicious_blockchain
